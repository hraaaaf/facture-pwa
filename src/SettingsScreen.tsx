  ...
  return (
    <main className="premium-settings-screen">
      ...
      <header className="settings-topbar">
        ...
        <span className="settings-top-spacer" />
      </header>

      <section className="settings-hero">
        <div className="settings-brand-mark">
          {draft.logoDataUrl ? <img src={draft.logoDataUrl} alt="Logo société" /> : <span>{draft.brand.slice(0, 1) || 'F'}</span>}
        </div>
        <div>
          <span className="settings-kicker">Identité active</span>
          <strong>{draft.name || 'Votre entreprise'}</strong>
          <small>{draft.brand || 'Marque à renseigner'}</small>
        </div>
      ...
      <SettingsSection eyebrow="Documents" title="Fiscalité & rendu PDF">
        ...
        <div className="settings-choice-block">
          <div>
            <strong>Modèle PDF par défaut</strong>
        ...
        <div className="numbering-preview">
          <div>
            <span className="settings-kicker">Numérotation</span>
            <strong>DEV · F · BL · BC</strong>
            <small>La séquence irréversible sera activée dans le lot moteur métier.</small>
          </div>
          <span className="settings-soon-chip">Prochain lot</span>
        </div>
      </SettingsSection>
      ...
      <nav className="settings-save-bar">
        <button className="settings-secondary" onClick={onBack}>Fermer</button>
        <button className="settings-primary" onClick={() => void save()}>Enregistrer les réglages</button>
      </nav>
    </main>
  )
}
