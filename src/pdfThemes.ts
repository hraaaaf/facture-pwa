...
const THEMES: Record<CustomTheme, ThemeSpec> = {
  majestic: { id: 'majestic', variant: 'band', primary: [7, 37, 70], secondary: [255, 255, 255], accent: [200, 151, 53], paper: [255, 255, 255], muted: [92, 101, 112] },
  lumiere: { id: 'lumiere', variant: 'editorial', primary: [28, 28, 28], secondary: [255, 255, 255], accent: [164, 153, 139], paper: [252, 250, 247], muted: [105, 101, 95] },
  terracotta: { id: 'terracotta', variant: 'sidebar', primary: [172, 77, 46], secondary: [255, 248, 241], accent: [223, 169, 126], paper: [255, 252, 249], muted: [113, 91, 81] },
  innova: { id: 'innova', variant: 'corporate', primary: [10, 49, 87], secondary: [245, 249, 252], accent: [49, 101, 151], paper: [255, 255, 255], muted: [93, 109, 123] },
  platine: { id: 'platine', variant: 'split', primary: [18, 18, 18], secondary: [243, 243, 241], accent: [151, 151, 145], paper: [255, 255, 255], muted: [99, 99, 95] },
  atlas: { id: 'atlas', variant: 'ornate', primary: [4, 82, 59], secondary: [255, 253, 247], accent: [183, 141, 46], paper: [255, 254, 249], muted: [79, 103, 94] }
}
...
const rows = (document: CommercialDocument) => document.lines.map(line => {
  ...
  return showPricing(document) ? [...base, money(line.unitPriceHT), money(lineTotalHT(line))] : base
})
...
const drawIdentity = (pdf: jsPDF, company: CompanySettings, x: number, y: number, color: RGB, maxWidth = 75) => {
...
const drawHeader = (pdf: jsPDF, document: CommercialDocument, company: CompanySettings, theme: ThemeSpec) => {
  ...
  if (theme.variant === 'band') {
    ...
    drawIdentity(pdf, company, 14, 13, theme.secondary)
    setColor(pdf, theme.secondary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(24); pdf.text(title, 194, 24, { align: 'right' })
    setColor(pdf, theme.accent); pdf.setFontSize(10); pdf.text(number, 194, 36, { align: 'right' })
    setColor(pdf, theme.secondary); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); pdf.text(formattedDate(document.date), 194, 44, { align: 'right' })
    setFill(pdf, theme.accent); pdf.rect(0, 58, 210, 1.2, 'F')
    return 70
  }

  if (theme.variant === 'editorial') {
    drawIdentity(pdf, company, 14, 12, theme.primary)
    setColor(pdf, theme.primary); pdf.setFont('times', 'normal'); pdf.setFontSize(29); pdf.text(title, 14, 54)
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(8); pdf.text(number, 194, 45, { align: 'right' })
    pdf.setFont('helvetica', 'normal'); pdf.text(formattedDate(document.date), 194, 53, { align: 'right' })
  ...
  if (theme.variant === 'sidebar') {
    ...
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11); pdf.text(pdf.splitTextToSize(company.name, 31), 7, 48)
    ...
    setColor(pdf, [44, 38, 34]); pdf.setFont('times', 'bold'); pdf.setFontSize(24); pdf.text(title, 52, 25)
    setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9.5); pdf.text(number, 194, 25, { align: 'right' })
    setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); pdf.text(formattedDate(document.date), 194, 34, { align: 'right' })
    return 44
  }

  if (theme.variant === 'corporate') {
    drawIdentity(pdf, company, 13, 12, theme.primary)
    setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(22); pdf.text(title, 195, 22, { align: 'right' })
    pdf.setFontSize(10); pdf.text(number, 195, 32, { align: 'right' })
    setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); pdf.text(formattedDate(document.date), 195, 40, { align: 'right' })
  ...
  if (theme.variant === 'split') {
    ...
    drawIdentity(pdf, company, 13, 13, theme.secondary, 68)
    setColor(pdf, theme.primary); pdf.setFont('times', 'normal'); pdf.setFontSize(26); pdf.text(title, 194, 22, { align: 'right' })
    setColor(pdf, theme.accent); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(10); pdf.text(number, 194, 34, { align: 'right' })
    setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); pdf.text(formattedDate(document.date), 194, 43, { align: 'right' })
    return 65
  }

  drawIdentity(pdf, company, 14, 12, theme.primary)
  setColor(pdf, theme.primary); pdf.setFont('times', 'bold'); pdf.setFontSize(25); pdf.text(title, 194, 20, { align: 'right' })
  setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(10); pdf.text(number, 194, 31, { align: 'right' })
  setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); pdf.text(formattedDate(document.date), 194, 39, { align: 'right' })
...
const drawInfo = (pdf: jsPDF, document: CommercialDocument, theme: ThemeSpec, startY: number) => {
  ...
  setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(6.5)
  ...
  pdf.setFont('helvetica', 'normal'); pdf.setFontSize(6.2); setColor(pdf, theme.muted)
...
const drawSummaryAndSignatures = (pdf: jsPDF, document: CommercialDocument, company: CompanySettings, theme: ThemeSpec, startY: number) => {
  const priced = showPricing(document)
  const totals = documentTotals(document)
  const { left, right, width } = contentBounds(theme)
  let y = Math.max(startY + 9, 176)
  const required = priced ? 61 : 38
  if (y + required > 262) {
  ...
  if (priced) {
    ...
    setFill(pdf, theme.secondary); setDraw(pdf, theme.accent)
    pdf.roundedRect(boxX, y, boxWidth, 36, 2, 2, 'FD')
    setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7)
    pdf.text('Total HT', boxX + 5, y + 9); pdf.text(`${money(totals.totalHT)} MAD`, right - 5, y + 9, { align: 'right' })
    pdf.text('TVA', boxX + 5, y + 18); pdf.text(`${money(totals.totalVAT)} MAD`, right - 5, y + 18, { align: 'right' })
    setFill(pdf, theme.primary); pdf.rect(boxX, y + 24, boxWidth, 12, 'F')
    setColor(pdf, theme.secondary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(8.5)
    pdf.text('TOTAL TTC', boxX + 5, y + 32); pdf.text(`${money(totals.totalTTC)} MAD`, right - 5, y + 32, { align: 'right' })

    setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(6.5); pdf.text('MONTANT EN LETTRES', left, y + 4)
    pdf.setFont('times', 'italic'); pdf.setFontSize(7.4); pdf.text(pdf.splitTextToSize(amountToFrenchDirhams(totals.totalTTC), Math.max(55, width - boxWidth - 12)), left, y + 12)
    y += 46
  }

  setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(7)
...
const addPageNumbers = (pdf: jsPDF, theme: ThemeSpec) => {
  ...
  for (let page = 1; page <= count; page += 1) {
    pdf.setPage(page); setColor(pdf, theme.muted); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(6)
...
const columnStyles = (priced: boolean, width: number) => {
  ...
  return {
    ...
    4: { cellWidth: width * 0.17 }
  }
}

const buildCustomTheme = (document: CommercialDocument, company: CompanySettings, themeId: CustomTheme) => {
  ...
  const tableY = drawInfo(pdf, document, theme, headerEnd)
  const { left, right, width } = contentBounds(theme)
  const priced = showPricing(document)
  ...
  autoTable(pdf, {
    ...
    styles: { font: 'helvetica', fontSize: 7.2, cellPadding: 3.2, minCellHeight: 11, valign: 'middle', halign: 'center', overflow: 'linebreak', textColor: theme.primary, lineColor: theme.accent, lineWidth: theme.variant === 'editorial' ? 0.12 : 0.18 },
    ...
    columnStyles: columnStyles(priced, width),
    margin: { left, right: 210 - right, top: 18, bottom: 32 },
    willDrawPage: data => {
      if (data.pageNumber > 1) {
        drawPageBase(pdf, theme)
        setColor(pdf, theme.primary); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(8)
        pdf.text(`${documentLabel(document.type)} ${displayNumber(document)} · suite`, left, 14)
      }
    },
    didDrawPage: () => footer(pdf, company, theme)
  })

  const finalY = (pdf as unknown as { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? tableY
  drawSummaryAndSignatures(pdf, document, company, theme, finalY)
  footer(pdf, company, theme)
  addPageNumbers(pdf, theme)
  return pdf
}

export const createThemedPdf = (document: CommercialDocument, company: CompanySettings, template: PremiumThemeId = 'premium') => {
  if (template === 'original' || template === 'premium') return createBasePdf(document, company, template)
  return buildCustomTheme(document, company, template)
}
...
export const downloadThemedPdf = (document: CommercialDocument, company: CompanySettings, template: PremiumThemeId = 'premium') =>
  createThemedPdf(document, company, template).save(themedPdfFileName(document, template))

export const shareThemedPdf = async (document: CommercialDocument, company: CompanySettings, template: PremiumThemeId = 'premium') => {
  const blob = createThemedPdf(document, company, template).output('blob')
  ...
  downloadThemedPdf(document, company, template)
  return false
}

export const printThemedPdf = (document: CommercialDocument, company: CompanySettings, template: PremiumThemeId = 'premium') => {
  const url = URL.createObjectURL(createThemedPdf(document, company, template).output('blob'))
  const printWindow = window.open(url, '_blank')
  if (!printWindow) { URL.revokeObjectURL(url); downloadThemedPdf(document, company, template); return }
