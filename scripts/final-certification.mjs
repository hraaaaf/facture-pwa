...
async function startDocument(page, type) {
  ...
  const dialog = page.getByRole('dialog', { name: 'Nouveau document' })
...
async function fillCurrent(page, { client = 'Client Certification', object = 'Objet certification', designation = 'Service Certification', qty = 1, pu = 100, vat = 20, date = '2026-08-26' } = {}) {
  ...
  const clientField = page.getByPlaceholder('Nom du client ou organisme')
  ...
  await page.getByLabel('Qté').fill(String(qty))
  await page.getByLabel('PU HT').fill(String(pu))
  await page.getByLabel('TVA %').fill(String(vat))
}
...
async function measureTouch(page, viewport) {
  await page.getByLabel('Nouveau document').click()
  const sheet = page.getByRole('dialog', { name: 'Nouveau document' })
  await sheet.waitFor()
  const closeBox = await sheet.getByRole('button', { name: 'Fermer' }).boundingBox()
...
try {
  ...
  // LOT 1: first invoice with reusable client snapshot.
  ...
  const clientDialog = page.getByRole('dialog', { name: 'Fiche client' })
  await clientDialog.waitFor()
  await clientDialog.getByLabel('Adresse').fill('Ancienne adresse certification')
  await clientDialog.getByLabel('ICE').fill('001122334455667')
  await clientDialog.getByLabel('IF').fill('99887766')
  await clientDialog.getByRole('button', { name: 'Mémoriser le client' }).click()
  ...
  // LOT 2: client autocomplete + catalog learned after finalization + profile update.
  await startDocument(page, 'FACTURE')
  ...
  assert.equal(await page.getByLabel('PU HT').inputValue(), '800')
  ok('Client autocomplete + catalogue runtime')
  await page.getByRole('button', { name: 'Fiche' }).click()
  const editClientDialog = page.getByRole('dialog', { name: 'Fiche client' })
  await editClientDialog.getByLabel('Adresse').fill('Nouvelle adresse certification')
  await editClientDialog.getByRole('button', { name: 'Mémoriser le client' }).click()
  ...
  // Snapshot historique must remain old after reusable client edit.
  await page.locator('.bottom-nav .nav-item').filter({ hasText: 'Historique' }).click()
  ...
  // Cancel 001, then 003 must not reuse any number.
  await page.locator('.bottom-nav .nav-item').filter({ hasText: 'Historique' }).click()
  ...
  await page.locator('.bottom-nav .nav-item').filter({ hasText: 'Accueil' }).click()
  const f3 = await simpleFinalize(page, 'FACTURE', 'F-2026-003')
  report.numbers.f3 = f3

  // Independent sequences + annual reset.
  ...
  // Restore v1 compatibility: documents/company restored, memory intentionally absent.
  ...
  page.once('dialog', dialog => dialog.accept())
  await settings.locator('input[type="file"][accept*="json"]').setInputFiles(v1Path)
  await page.getByText('Sauvegarde restaurée avec succès', { exact: true }).waitFor()
  ...
  // Restore v2 recovers memory too.
  page.once('dialog', dialog => dialog.accept())
  await settings.locator('input[type="file"][accept*="json"]').setInputFiles(backupPath)
  await page.getByText('Sauvegarde restaurée avec succès', { exact: true }).waitFor()
  ...
  // Exact-head tactile AFTER at all requested widths.
