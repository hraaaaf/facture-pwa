...
try{
  ...
  const before=await browser.newPage({viewport:{width:390,height:844}});await seed(before,baseline.url);check('baseline_manager_present',await before.getByRole('button',{name:/Clients & catalogue/}).count()===1,'current main must expose the Step 5 dedicated manager');await before.close()
